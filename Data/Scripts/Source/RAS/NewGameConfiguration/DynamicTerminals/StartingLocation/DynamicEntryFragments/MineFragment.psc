Scriptname RAS:NewGameConfiguration:DynamicTerminals:StartingLocation:DynamicEntryFragments:MineFragment extends MiscObject
...
Event RAS:NewGameConfiguration:DynamicTerminals:Base:DynamicEntriesTerminalScript.SelectedFragmentTriggered(RAS:NewGameConfiguration:DynamicTerminals:Base:DynamicEntriesTerminalScript akSender, var[] kArgs)
    If(kArgs[0] as Form == Self)
        ...
        If(!locationSpawnPointFinderQuestScript.MoveToLocation(locationSpawnPointFinderQuestScript.RandomMine.GetLocation(), !myShipVendorScript.NoShipSelected))
